...
if (ch != "") {
	...
	if ((is_numb || is_cyrillic || is_english || is_special || code == 63) && string_count_ext(temp_text, max_width) <= 4) {
...
if (Obj_copy.zcp >= 1) {
	Obj_copy.zcp = 0
	if (lo == 0 && st == 1) {
		clipboard_set_text(Obj_shif_unshif.shifr(text_input))
	}
	else if (lo == 1 && st == 1) {
		clipboard_set_text(Obj_shif_unshif.deshifr(text_input))
	}
	else {
